        ...
        for (let j = 0; j < 109; j++) {
            for (let k = 0; k < 63; k++) {
                if (c[cy+i] && c[cy+i][j] && c[cy+i][j][k]) {
                    ...
                    for (let l = 0; l < 4; l++) {
                         for (let m = 0; m < 4; m++) {
                             var xx = x+(l*0.01*Math.round(Math.random()));
                             var yy = y+(m*0.02*Math.round(Math.random()));
...
float roundedRectangle (vec2 uv, vec2 pos, vec2 size, float radius, float thickness) {
    ...
    void main(void) {
        float t = time * 1e-3;
        gl_Position = vec4(coordinates.x * 1.5, coordinates.y * 1.5, 0.0, 1.0);
...
float vig = (roundedRectangle(gl_Position.xy * 0.15, vec2(0.0, 0.0), vec2(2.0, 1.96) * 0.0712, 0.001, 0.05) + 0.0);
    ...
    gl_PointSize *= floor(vig) * 6.;
        gl_Position.xy += vec2(noise(vec2(pos.x, t*1e1)), noise(vec2(pos.x+1000., t*1e1)))*0.25;
